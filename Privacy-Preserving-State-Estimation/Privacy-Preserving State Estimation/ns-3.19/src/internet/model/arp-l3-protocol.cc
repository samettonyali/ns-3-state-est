...
/** new module, for arp in preq
*/
void
ArpL3Protocol::AcceptArpFromHwmp( Ptr<MeshPointDevice> mp, Ipv4Address ip, Mac48Address mac) 
{
   // the mapping address is valid in here since the signature has been verified in security module, 
   // search in the ARP table, if not found add, else update
  
   Ptr<ArpCache> cache = FindCache (mp);
   NS_LOG_LOGIC (" Accept ARP address mapping < " << ip << " , " << mac << " > , I am " << mp->GetAddress () << " " << cache );
   
   ArpCache::Entry *entry = cache->Lookup (ip);
   if (entry == 0 ) 
     {
